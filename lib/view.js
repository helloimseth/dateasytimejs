...
Picker.prototype._newPickerBox = function () {
  ...
  this._attachHiddenInputTo(pickerBox);
...
Picker.prototype.selectTimeDropdownValue = function (event) {
    ...
    if (attr == 'minute') {
      ...
      var ampm = document.getElementById('dateasytime-time-picker-ampm').selectedIndex;
      this.returnVals[attr] = parseInt(val) + (12 * ampm);
    } else if (idx === 0) {
...
Picker.prototype._attachHiddenInputTo = function (pickerBox) {
    var input = document.createElement('input');
    input.setAttribute('type', 'hidden');
    input.setAttribute('name', this.inputName);
    input.id = 'dateasytime-picker-input';

    pickerBox.appendChild(input);
};

Picker.prototype.updateInput = function (input) {
  if (!input) {
    input = document.getElementById('dateasytime-picker-input');
  }

  console.log(this._valString());
